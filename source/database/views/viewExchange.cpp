...
namespace views
{
  ...
  void CViewExchange::processActiveStatus()
  {
    activeStatus_ = database::tbl_imm_exchanges::activeStatuses(application(), ID());
  }
  ...
  std::set<businessTransaction_e> CViewExchange::processAllowedBusinessTransactions() const
  {
  ...
  bool CViewExchange::processAllowTransaction(businessTransaction_e bt) const
  {
    bool rv = false;
  ...
  void CViewExchange::processBusinessTransaction(businessTransaction_e businessTransaction)
  {
  ...
  void CViewExchange::processClearData() noexcept
  {
    record_.ID = 0;
    record_.shortText.clear();
    record_.longText.clear();
    record_.suffix.clear();
    record_.countryID = 0;
  }
  ...
  void CViewExchange::processLoadData()
  {
    database::tbl_imm_exchanges::readRecord(application(), record_);
  }

  /// @brief      Saves data to the database.
  /// @version    2023-07-21/GGB - Function created.

  void CViewExchange::processSaveData()
  {
    if (ID() == 0)
    {
      database::tbl_imm_exchanges::createRecord(application(), record_);
      updateStatus(SC_CREATED);
    }
    else
    {
      database::tbl_imm_exchanges::updateRecord(application(), record_);
    }
  }
}
